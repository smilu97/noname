...
class GameComponentButton(GameComponent):
	def __init__(self, owner, onClick=0, senseRect = [0.0,0.0,0.0,0.0]):
		...
		self.senseRect = np.array(senseRect)
		...
		if self.onClick != 0 :
			for event in self.owner.owner.events :
				if event.type == MOUSEBUTTONDOWN :
					...
					worldSenseRect = self.senseRect + np.array([self.owner.position[0], self.owner.position[1], 0, 0])
					if self.senseRect[0] <= mousepos[0] and mousepos[0] <= self.senseRect[0] + self.senseRect[2] and \
						self.senseRect[1] <= mousepos[1] and mousepos[1] <= self.senseRect[1] + self.senseRect[3] :
						self.onClick()
					break
	def Render(self):
		pass